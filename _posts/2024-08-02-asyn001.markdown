...
```
1. 步骤如下：       
    - 自动执行**lock.unlock()**，然后进入阻塞;          
    - 当**notify_all()**或**notify_one()**被执行，线程阻塞将被解开。或者线程将被意外解开阻塞;               
    - 当阻塞解开，执行**lock.lock()**.      
...
2. 如果**t1**进入等待，然后**t2**进行对**t1**唤醒，唤醒分为两步：    
   - 设置变量**dataReady**为**true**;    
   - **conVar**执行唤醒;   
   - **t1**中的**wait**结束，跳出循环。   
     
3. 如果**t2**先执行，**t1**会被**mutex_**阻塞。    
   - 当设置变量**dataReady**为**true**完成，并且**lock_guard**完成析构;    
   - 此时**t1**才能解开**mutex**的阻塞;     
   - **unique_lock**加锁成功;     
   - **conVar.wait**无法进入循环;    
   - 任务在加锁的情况下执行;     
