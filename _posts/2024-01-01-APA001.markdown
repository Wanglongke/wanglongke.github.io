...
有理数$\frac{54}{4}=1.275*10^{1}=1.10011*2^{3}$可以用有限位表示，无理数$\sqrt{2}$则需要无穷位表示。可以使用$\sigma \in \{-1,0,1\}$表示符号。把具有这种形式$\sigma(1.c) * 2^{p}$表示的数的集合$B$称为二进制科学计数。   
  
如果$x,y \in B$, 则$x+y, x-y, x*y$ 都属于$B$, 但是除法是不包含的。因为除法会产生无穷位的表示。例如，$1=1.0*2^{0}$ 和 $3=1.1*2^{1}$, 但是$1/3$需要无穷位表示。 
   
二进制科学计数的具体实现：   
$r=1.c*2^{p}$, 其中$c$有$n$位，$c_{0},...c_{n-1}$, 其中最后一位为$c_{n-1}=1$   
其可改写为：   
$r=\hat{c}*2^{p-n}=\hat{c}*2^{b}$ 其中$\hat{c}$ 是$(n+1)$位整数， 其首位与尾位都是1   
...
如果 $u=0$, $x*y=1.v*2^{p+q}$
如果 $v=0$, $x*y=1.u*2^{p+q}$   
如果 $u>0, v>0$， $u$有$n>0$位， $v$有$m>0$位，则:   
$$x*y=1.u*2^{p}*1.v*2^{q}=\hat{u}*2^{p-n}*\hat{v}*2^{q-m}=\hat{u}*\hat{v}*2^{p-n+q-m}=\hat{w}*2^{p-n+q-m}$$  
其中$\hat{w}=\hat{u}*\hat{v}$ 是奇数乘积。$\hat{u}$有$n+1$位，$\hat{v}$有$m+1$位，所以$\hat{w}$是一个奇数，其有$n+m+1$或$n+m+2$位。   
...
则$\hat{w}$是一个$(l+1)$位的奇数，$\hat{w}=1w_{0}...w_{l-1}=1.w_{0}...w_{l-1}*2^{l}=1.w*2^{l}, w_{l-1}=1$   
