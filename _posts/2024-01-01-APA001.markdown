...
$$r=\hat{c}*2^{p-n}=\hat{c}*2^{b}$$    
    
其中$\hat{c}$ 是$(n+1)$位整数， 其首位与尾位都是1   
那么$r$可表示为：符号$\sigma$, $n+1$位整数$\hat{c}$, 指数$b$   
...
$$x*y=1.u*2^{p}*1.v*2^{q}=\hat{u}*2^{p-n}*\hat{v}*2^{q-m}=\hat{u}*\hat{v}*2^{p-n+q-m}=\hat{w}*2^{p-n+q-m}$$  

其中  
$$\hat{w}=\hat{u}*\hat{v}$$   

是奇数乘积（$u$和$v$的最后一位都是1）。$\hat{u}$有$n+1$位，$\hat{v}$有$m+1$位，所以$\hat{w}$是一个奇数，其有$n+m+1$或$n+m+2$位。   
...
则$\hat{w}$是一个$(l+1)$位的奇数，  

$$\hat{w}=1w_{0}...w_{l-1}=1.w_{0}...w_{l-1}*2^{l}=1.w*2^{l}, w_{l-1}=1$$   
    
$$x*y=\hat{w}*2^{p-n+q-m}=1.w*2^{p-n+q-m+l}=1.w*2^{p+q+c}=1.w*2^{r}=z$$   
其中$r=p+q+c$
...
$$\hat{u}*2^{p-n} + \hat{v}*2^{q-m} = (\hat{u}*2^{d}+\hat{v}) *2^{q-m}= \hat{w}*2^{q-m}$$   

其中$d=(p-n)-(q-m)$.      
整数$\hat{u}*2^{d}$ 是偶数，因为$\hat{u}$是奇数且$d>0$. 整数$\hat{v}$是奇数. 因此$\hat{w}$是奇数。

$$x+y=\hat{w}*2^{q-m}=1.w*2^{q-m+l}=1.w*2^{r}=z$$   

其中$r=q-m+l$, 其中$l$是$\hat{w}$首1-bit的位置(索引)。 这个加法的实现包含计算$\hat{u}*2^{d}$, 其用**位左移**运算实现。然后加上$\hat{v}$，然后就可以确定$\hat{w}$的首1-bit位置了。
...
$$\hat{u}*2^{p-n} + \hat{v}*2^{q-m} = (\hat{u}+\hat{v}*2^{d}) *2^{p-n}= \hat{w}*2^{p-n}$$   

其中$d=(q-m)-(p-n)$.    
整数$\hat{v}*2^{d}$是偶数因为$\hat{v}$是奇数且$d>0$. 整数$\hat{u}$是奇数. 因此$\hat{w}$是奇数。     

$$x+y=\hat{w}*2^{p-n}=1.w*2^{p-n+l}=1.w*2^{r}=z$$

其中$r=p-n+l$, 其中$l$是$\hat{w}$首1-bit的位置(索引)。这个加法的实现包含计算$\hat{v}*2^{d}$, 其用**位左移**运算实现。然后加上$\hat{u}$，然后就可以确定$\hat{w}$的首1-bit位置了。
...
$$\hat{u}*2^{p-n} + \hat{v}*2^{q-m} = (\hat{u}+\hat{v}) *2^{q-m}= \hat{w}*2^{q-m}$$   

整数$\hat{u}$和$\hat{v}$是奇数，因此$\hat{w}$是偶数. 设$l$是$\hat{w}$首1-bit的位置(索引)，设t是$\hat{w}$尾1-bit的位置。其中需要$l>=t>0$，可以对$hat{w}$位右移$t$位获得一个奇数$\hat{w}$, 此时$\hat{w}$首1-bit的位置(索引)$f=l-t$:     

$$x+y=\hat{w}*2^{q-m}=w^{*}*2^{q-m+t}=1.w*2^{q-m+t+f}=1.w*2^{q-m+l}=1.w*2^{r}=z$$

其中$r=q-m+l$。小数部分$w$(表示为整数)有$l-t$位，加法的实现涉及整数加法$\hat{w}=\hat{u}+\hat{v}$，然后确定$l$和$t$，$\hat{w}$位右移$t$位获得$w^{*}$, 最终得到$z=1.w*2^{r}$，无符号整数$w^{*}$和指数$b=q-m+t$.
