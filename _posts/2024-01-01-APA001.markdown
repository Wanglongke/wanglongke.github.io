...
如果$\quad ac-b^{2}>0$，则$F(s,t)$有全局最小值，当$\nabla F(s,t)=(0,0)$. 此时解$(\tilde{s}, \tilde{t})$分别代表两条直线距离最近是的参数。  
接下来求解：
...
$$\left( \begin{array}{c} \tilde{s} \\ \tilde{t} \end{array} \right) = \frac{1}{ac-b^{2}} \left( \begin{array}{c} be-cd \\ ae-bd \end{array}\right)$$
